            ...
            <div key={src} className="mb-6 break-inside-avoid overflow-hidden rounded-2xl glass-panel group relative">
              {src.endsWith('.mp4') ? (
                <video
                  ...
                  muted
                  playsInline
                  className={`w-full h-auto object-cover transition-all duration-1000 group-hover:scale-105 ${loaded[src] ? 'opacity-100 blur-0' : 'opacity-0 blur-md'}`}
                  onLoadedData={() => setLoaded(prev => ({ ...prev, [src]: true }))}
